...
typedef enum
{
    ...
    GAP_PARAM_EXT_CONN_V2              = 0x23a, //!< Use LE Extended Create Connection V2, create an ACL connection between a periodic advertiser and a synchronized device.
    GAP_PARAM_EXT_CONN_V2_ADV_HANDLE   = 0x23b, //!< Parameters of LE Extended Create Connection V2, Advertising_Handle identifying the periodic advertising train. Range: 0x00 to 0xEF or 0xFF(parameter not used).
    GAP_PARAM_EXT_CONN_V2_SUBEVENT     = 0x23c, //!< Parameters of LE Extended Create Connection V2, Subevent where the connection request is to be sent. Range: 0x00 to 0x7F or 0xFF(parameter not used).
} T_GAP_LE_PARAM_TYPE;
