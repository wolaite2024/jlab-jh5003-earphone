...
//gap_pa_adv.h
#define GAP_MSG_LE_PERIODIC_ADV_SUBEVENT_DATA_REQUEST_INFO                      0x81 //!<Notification msg type for le pawr subevent data request
#define GAP_MSG_LE_PERIODIC_ADV_RESPONSE_REPORT_INFO                            0x82 //!<Notification msg type for le pawr response report
//gap_pa_sync.h
#define GAP_MSG_LE_PERIODIC_ADV_RESPONSE_DATA_SET                               0x83 //!<Response msg type for le_pa_sync_set_periodic_adv_response_data
#define GAP_MSG_LE_PERIODIC_SYNC_SUBEVENT_SET                                   0x84 //!<Response msg type for le_pa_sync_set_periodic_sync_subevent
//gap_pa_adv.h
#define GAP_MSG_LE_PERIODIC_ADV_SUBEVENT_DATA_SET                               0x85 //!<Response msg type for le_pa_adv_set_periodic_adv_subevent_data
...
typedef struct
{
    ...
    bool        v2_event;                   /**< Flag indicating HCI event version.
                                                 true: v2 event.
                                                 false: v1 event. */
    uint16_t    periodic_event_counter;     /**< The value of paEventCounter.
                                                 Valid if v2_event is true, otherwise invalid.
...
typedef struct
{
    ...
    bool                v2_event;                   /**< Flag indicating HCI event version.
                                                         @arg true: v2 event.
                                                         @arg false: v1 event. */
    uint8_t             num_subevents;              /**< Number of events.
                                                         Valid if v2_event is true, otherwise invalid.
                                                         @arg Range: 0x01 to 0x80.
                                                         @arg 0x00: No subevents. */
    uint8_t             subevent_interval;          /**< Subevent interval.
                                                         Valid if v2_event is true and num_subevents is not equal to 0x00, otherwise invalid.
                                                         @arg Range: 0x06 to 0xFF.
                                                         @arg Time = N * 1.25ms.
                                                         @arg Time Range: 7.5 ms to 318.75 ms. */
    uint8_t             rsp_slot_delay;             /**< Response slot delay.
                                                         Valid if v2_event is true and num_subevents is not equal to 0x00, otherwise invalid.
                                                         @arg Range: 0x01 to 0xFE.
                                                         @arg Time = N * 1.25 ms.
                                                         @arg Time Range: 1.25 ms to 317.5 ms. */
    uint8_t             rsp_slot_spacing;           /**< Response slot spacing.
                                                         Valid if v2_event is true and num_subevents is not equal to 0x00, otherwise invalid.
                                                         @arg Range: 0x02 to 0xFF.
                                                         @arg Time = N * 0.125 ms.
                                                         @arg Time Range: 0.25 ms to 31.875. */
...
typedef struct
{
    uint16_t            cause;
    uint8_t             adv_handle;
} T_LE_PA_SET_PERIODIC_ADV_SUBEVENT_DATA_RSP;
...
} T_LE_PERIODIC_ADV_SUBEVENT_DATA_REQUEST_INFO;
...
typedef struct
{
    ...
    uint8_t             tx_power;   /**< Range: -127 to +20 dBm
                                         0x7F: Tx Power information not available */
    uint8_t             rssi;       /**< Range: -127 to +20 dBm
                                         0x7F: RSSI is not available */
    T_GAP_PERIODIC_ADV_REPORT_CTE_TYPE          cte_type;
    ...
    uint8_t             data_len;   /**< Length of the Data field. */
    uint8_t            *p_data;     /**< Periodic advertising response data. */
} T_LE_PERIODIC_ADV_RESPONSE_REPORT_INFO;
...
typedef struct
{
    ...
    uint16_t            sync_handle;
    uint16_t            cause;
} T_LE_PA_SET_PERIODIC_ADV_RESPONSE_DATA_RSP;
...
typedef struct
{
    ...
    uint16_t            sync_handle;
    uint16_t            cause;
} T_LE_PA_SET_PERIODIC_SYNC_SUBEVENT_RSP;
