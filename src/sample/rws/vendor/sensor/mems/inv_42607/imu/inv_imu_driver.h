 ...
 */
#if (SENSOR_25HZ == 1)
#define FIFO_MIRRORING_SIZE 128 // packet size * max_count = 4kB
#else
#define FIFO_MIRRORING_SIZE 256 // packet size * max_count = 4kB
#endif
...
enum inv_imu_sensor
{
    INV_SENSOR_ACCEL,               /**< Accelerometer (UI control path) */
    INV_SENSOR_GYRO,                /**< Gyroscope (UI control path) */
    INV_SENSOR_FSYNC_EVENT,         /**< Used by UI control layer */
    INV_SENSOR_TEMPERATURE,         /**< Chip temperature, enabled by default. However, it will be reported only if Accel and/or Gyro are also enabled.
                                                  The Temperature's ODR (Output Data Rate) will match the ODR of Accel or Gyro, or the fastest if both are enabled*/
    INV_SENSOR_DMP_PEDOMETER_EVENT, /**< Pedometer: step is detected */
    INV_SENSOR_DMP_PEDOMETER_COUNT, /**< Pedometer: step counter */
    INV_SENSOR_DMP_TILT,            /**< Tilt */
    INV_SENSOR_DMP_FF,              /**< Free Fall */
...
typedef struct
{
    ...
    inv_imu_sensor_data gyro_acc_sensor_data[FIFO_DEPTH];
} inv_imv_report_data;
...
{
    /* Warning : this field MUST be the first one of struct inv_imu_device */
    struct inv_imu_transport transport;

    /* callback executed by inv_imu_get_data_from_fifo function for each data packet extracted from fifo or inv_imu_get_data_from_registers read data from register,
    This field may be NULL if inv_imu_get_data_from_fifo/inv_imu_get_data_from_registers is not used by application. */
    ...
    uint8_t endianness_data;                             /**< internal status of data endianness mode to report correctly data */
    uint8_t fifo_highres_enabled;                        /**< FIFO packets are 20 bytes long */

    /**< Data are get from FIFO or from sensor registers. By default Fifo is used*/
    INV_IMU_FIFO_CONFIG_t fifo_is_used;
 ...
 *  @warning inv_device::register_cache::pwr_mngt_0_reg is modified by this function
 */
 ...
 *  @return 0 on success, negative value on error.
 *  @warning inv_device::register_cache::accel_cfg_0_reg is modified by this function
 */
 ...
 *  @return 0 on success, negative value on error.
 *  @warning inv_device::register_cache::gyro_cfg_0_reg is modified by this function
 */
 ...
 *  @sa ACCEL_CONFIG0_FS_SEL_t.
 *  @return 0 on success, negative value on error.
 *  @warning inv_device::register_cache::accel_cfg_0_reg is relied upon by this function
 */
 ...
 *  @sa GYRO_CONFIG0_FS_SEL_t.
 *  @return 0 on success, negative value on error.
 *  @warning inv_device::register_cache::gyro_cfg_0_reg is relied upon by this function
 */
...
/** @brief Enable Fifo decimation
*  @return 0 on success, negative value on error.
*/
int inv_imu_enable_fifo_decimation(struct inv_imu_device *s);

/** @brief Disable Fifo decimation
*  @return 0 on success, negative value on error.
*/
int inv_imu_disable_fifo_decimation(struct inv_imu_device *s);

/** @brief Configure Fifo decimation
*  @param[in] requested decimation factor value from 2 to 256
*  @return 0 on success, negative value on error.
*/
int inv_imu_configure_fifo_decimation(struct inv_imu_device *s, FIFO_DEC_CFG_FACTOR_t dec_factor);
...
int inv_imu_dump_reg(struct inv_imu_device *s);
