...
{
#if 0
    Qmi8658GyrRange_32dps = 0 << 4,
    Qmi8658GyrRange_64dps = 1 << 4,
    Qmi8658GyrRange_128dps = 2 << 4,
    Qmi8658GyrRange_256dps = 3 << 4,
    Qmi8658GyrRange_512dps = 4 << 4,
    Qmi8658GyrRange_1024dps = 5 << 4,
    Qmi8658GyrRange_2048dps = 6 << 4,
    Qmi8658GyrRange_4096dps = 7 << 4
#else
...
enum Qmi8658_AeOdr
{
    Qmi8658AeOdr_1Hz = 0x00,
    Qmi8658AeOdr_2Hz = 0x01,
    Qmi8658AeOdr_4Hz = 0x02,
    Qmi8658AeOdr_8Hz = 0x03,
    Qmi8658AeOdr_16Hz = 0x04,
    Qmi8658AeOdr_32Hz = 0x05,
    Qmi8658AeOdr_64Hz = 0x06,
    Qmi8658AeOdr_128Hz = 0x07,
...
enum Qmi8658_MagOdr
{
    Qmi8658MagOdr_1000Hz = 0x00,
    Qmi8658MagOdr_500Hz = 0x01,
    Qmi8658MagOdr_250Hz = 0x02,
    Qmi8658MagOdr_125Hz = 0x03,
    Qmi8658MagOdr_62_5Hz = 0x04,
    Qmi8658MagOdr_31_25Hz = 0x05
};
...
typedef struct
{
    ...
    float line_accel[3];//line_accel
} qmi_imu_algo_data;

typedef uint8_t(*P_QMI8658_I2C_READ_CB)(uint8_t reg, uint8_t *buf, uint32_t rlen);
typedef uint8_t(*P_QMI8658_I2C_WRITE_CB)(uint8_t reg, uint8_t value);
// typedef void(*P_QMI8658_I2C_WRITE_REGS_CB)(uint8_t reg, const uint8_t *buf, uint32_t len);

typedef struct
{
    P_QMI8658_I2C_READ_CB qst_sw_read_reg;
    P_QMI8658_I2C_WRITE_CB qst_sw_write_reg;
    // P_QMI8658_I2C_WRITE_REGS_CB qst_sw_write_regs;
} T_QMI8658_IF;
...
extern void Qmi8658_read_xyz(qmi_imu_sensor_data *dat);
...
extern void Qmi8658_enableWakeOnMotion(enum Qmi8658_Interrupt int_set,
                                       enum Qmi8658_WakeOnMotionThreshold threshold, unsigned char blankingTime);
extern void Qmi8658_disableWakeOnMotion(void);
extern void Qmi8658_config_am(void);
extern void Qmi8658_enable_am(void);
#if defined(QMI8658_USE_FIFO)
extern void Qmi8658_config_fifo(unsigned char watermark, enum Qmi8658_FifoSize size,
                                enum Qmi8658_FifoMode mode, enum Qmi8658_fifo_format format);
extern void Qmi8658_enable_fifo(void);
#endif
extern void Qmi8658_send_ctl9cmd(enum Qmi8658_Ctrl9Command cmd);
extern void Qmi8658_do_selftest(void);
