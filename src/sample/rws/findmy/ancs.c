...
/** End of PERIPH_ANCS_Exported_Variables
    * @}
    */
extern void *audio_io_queue_handle;  //!< Event queue handle
extern void *audio_evt_queue_handle;   //!< IO queue handle
/*============================================================================*
 *                              Functions
 *============================================================================*/
/** @defgroup PERIPH_ANCS_Exported_Functions ANCS Exported Functions
   * @{
   */
static void ancs_send_msg_to_app(uint8_t conn_id)
{
    T_IO_MSG io_msg;
    ...
    if (ret == false)
    {
        APP_PRINT_ERROR3("ancs_send_msg_to_app failed, type = %x, subtype = %x, param = %x",
                         io_msg->type, io_msg->subtype, io_msg->u.param);
    }
}

static void app_handle_notification_attribute_data(T_APP_ANCS_LINK *p_ancs_link)
{
    if (p_ancs_link->notification_attr.attribute_id != DS_NOTIFICATION_ATTR_ID_NEGATIVE_ACTION_LABEL)
    {
        ...
        if (p_ancs_link->notification_attr.attribute_id == DS_NOTIFICATION_ATTR_ID_APP_IDENTIFIER)
        {
            ...
            APP_PRINT_INFO1("app_handle_notification_attribute_data: app identifiter %s",
                            TRACE_STRING(p_value));
#endif
            //wechat
            if (memcmp(p_value, "com.tencent.xin", 16) == 0)
            {
                p_ancs_link->app_type = 1;
            }
            else if (memcmp(p_value, "com.apple.MobileSMS", 20) == 0)
            {
                p_ancs_link->app_type = 2;
            }
            else if (memcmp(p_value, "com.apple.mobilephone", 22) == 0)
            {
                p_ancs_link->app_type = 3;
            }
            else if (memcmp(p_value, "com.tencent.mqq", 16) == 0)
            {
                p_ancs_link->app_type = 4;
            }
            else if (memcmp(p_value, "com.tencent.qq", 15) == 0)
            {
        ...
        {
            if ((p_ancs_link->app_type == 2) || (p_ancs_link->app_type == 5))
            {
                APP_PRINT_INFO1("app_handle_notification_attribute_data: message %s", TRACE_STRING(p_value));
            }
        }
        ...
        {
#if F_BT_ANCS_CLIENT_DEBUG
            APP_PRINT_INFO1("app_handle_notification_attribute_data: parse notify attr date %s",
                            TRACE_STRING(p_value));
#endif
        }
#endif
...
static void app_parse_notification_attribute(T_APP_ANCS_LINK *p_ancs_link, uint8_t *p_data,
                                             uint8_t len)
{
    int i;
...
#if F_BT_ANCS_CLIENT_DEBUG
            APP_PRINT_INFO2("app_parse_notification_attribute: attribute_id %d, attribute_len %d",
...
#if F_BT_ANCS_GET_APP_ATTR
static void app_parse_app_attribute(T_APP_ANCS_LINK *p_ancs_link, uint8_t *p_data, uint8_t len)
{
    int i;
    ...
    for (i = 0; i < len; i++)
    {
        ...
        case DS_PARSE_APP_ATTRIBUTE_LEN2:
            ...
            p_ancs_link->current_len = 0;
#if F_BT_ANCS_CLIENT_DEBUG
            APP_PRINT_INFO2("app_parse_app_attribute: attribute_id %d, attribute_len %d",
 ...
 * @return void
 */
static void app_parse_data_soucre_notifications(uint8_t conn_id, uint8_t *p_data, uint8_t len)
{
    APP_PRINT_INFO2("app_parse_data_soucre_notifications: conn_id %d, len =%d", conn_id, len);
#if F_BT_ANCS_CLIENT_DEBUG
    APP_PRINT_INFO1("app_parse_data_soucre_notifications: data %b", TRACE_BINARY(len, p_data));
#endif
    T_APP_ANCS_LINK *p_ancs_link = &ancs_link_table[conn_id];

#if F_BT_ANCS_CLIENT_DEBUG
    APP_PRINT_INFO1("app_parse_data_soucre_notifications: m_parse_state %d",
...
static void app_parse_notification_source_data(uint8_t conn_id, uint8_t *p_data, uint8_t len)
{
    if (len == 8)
    {
        T_NS_DATA ns_data;
...
static T_APP_RESULT ancs_client_cb(T_CLIENT_ID client_id, uint8_t conn_id, void *p_data)
{
    ...
    case ANCS_CLIENT_CB_TYPE_DISC_STATE:
        {
            APP_PRINT_INFO1("ancs_client_cb: disc_state %d", p_cb_data->cb_content.disc_state);
    ...
    case ANCS_CLIENT_CB_TYPE_NOTIF_IND_RESULT:
        ...
        case ANCS_FROM_NOTIFICATION_SOURCE:
            APP_PRINT_INFO2("ancs_client_cb: ANCS_FROM_NOTIFICATION_SOURCE conn_id %d, length %d",
    ...
    case ANCS_CLIENT_CB_TYPE_WRITE_RESULT:
        {
            APP_PRINT_INFO2("ancs_client_cb: ANCS_CLIENT_CB_TYPE_WRITE_RESULT cause 0x%x, type %d",
    ...
    case ANCS_CLIENT_CB_TYPE_DISCONNECT_INFO:
        {
            ...
            APP_PRINT_INFO1("ancs_client_cb: ANCS_CLIENT_CB_TYPE_DISCONNECT_INFO conn_id = 0x%x", conn_id);
...
void ancs_handle_msg(T_IO_MSG *p_io_msg)
{
    ...
    {
        APP_PRINT_INFO1("ancs_handle_msg: ANCS_MSG_TYPE_GET_NOTIFI_ATTR notification_uid 0x%x",
                        ancs_msg.data.notifi_attr.notification_uid);
        ret = ancs_get_notification_attr(ancs_msg.data.notifi_attr.conn_id,
                                         ancs_msg.data.notifi_attr.notification_uid,
                                         ancs_msg.data.notifi_attr.attribute_ids,
                                         ancs_msg.data.notifi_attr.attribute_ids_len);
    }
    else if (ancs_msg.type == ANCS_MSG_TYPE_PERFORM_NOTIFI_ACTION)
    {
        APP_PRINT_INFO1("ancs_handle_msg: ANCS_MSG_TYPE_PERFORM_NOTIFI_ACTION notification_uid 0x%x",
                        ancs_msg.data.perform_action.notification_uid);
        ret = ancs_perform_notification_action(ancs_msg.data.perform_action.conn_id,
                                               ancs_msg.data.perform_action.notification_uid,
                                               ancs_msg.data.perform_action.action_id);
    }
#if F_BT_ANCS_GET_APP_ATTR
    else if (ancs_msg.type == ANCS_MSG_TYPE_GET_APP_ATTR)
    {
        ret = ancs_get_app_attr(ancs_msg.data.app_attr.conn_id,
                                ancs_msg.data.app_attr.app_identifier,
                                ancs_msg.data.app_attr.attribute_ids,
                                ancs_msg.data.app_attr.attribute_ids_len);
    }
#endif

    if (ret == false)
    {
        APP_PRINT_ERROR0("ancs_handle_msg: failed");
    }
}
...
void ancs_init(uint8_t link_num)
{
    ...
    ancs_link_table = os_mem_zalloc(OS_MEM_TYPE_DATA, ancs_link_number * sizeof(T_APP_ANCS_LINK));
    ...
    if (ret == false)
    {
        APP_PRINT_ERROR2("ancs_init: link_num %d, i 0x%x failed", link_num, i);
    }
