                           ...
                           const byte *priv)
{
...
#ifdef FREESCALE_LTC_ECC
    {
        ...
        ECPoint wc_pub;
        ret = nxp_ltc_curve25519(&wc_pub, priv, basepoint,
                                 kLTC_Weierstrass); /* input basepoint on Weierstrass curve */
        if (ret == 0)
        {
...
int wc_curve25519_make_key(WC_RNG *rng, int keysize, curve25519_key *key)
{
    ...
    return wc_curve25519_make_pub((int)sizeof key->p.point, key->p.point, sizeof key->k.point,
                                  key->k.point);
}
