...
static void st7265_reset_high(void)
{
...
static void st7265_reset_low(void)
{
...
//--------------------initial code-----------------------------------------//
static void st7265_gpio_init(void)
{
...
extern void LCDC_DMA_Infinite_Buf_Update(uint8_t *G1_SAR_buf, uint8_t *G2_SAR_buf);
static void st7265_dma_init(uint8_t *init_buffer)
{
    ...
    LCDC_SET_GROUP1_BLOCKSIZE(ST7265_800480_LCD_WIDTH * ST7265_DRV_PIXEL_BITS / 8);
    LCDC_SET_GROUP2_BLOCKSIZE(ST7265_800480_LCD_WIDTH * ST7265_DRV_PIXEL_BITS / 8);

    LCDC_DMALLI_InitTypeDef LCDC_DMA_LLI_Init = {0};

    LCDC_DMA_LLI_Init.g1_source_addr = (uint32_t)init_buffer;
    LCDC_DMA_LLI_Init.g2_source_addr = (uint32_t)((uint32_t)init_buffer + ST7265_800480_LCD_WIDTH *
                                                  ST7265_DRV_PIXEL_BITS / 8);
    LCDC_DMA_LLI_Init.g1_sar_offset = ST7265_800480_LCD_WIDTH * ST7265_DRV_PIXEL_BITS / 8 * 2;
    LCDC_DMA_LLI_Init.g2_sar_offset = ST7265_800480_LCD_WIDTH * ST7265_DRV_PIXEL_BITS / 8 * 2;

    LCDC_DMA_Infinite_Buf_Update((uint8_t *)init_buffer,
                                 (uint8_t *)init_buffer + ST7265_800480_LCD_WIDTH *
                                 ST7265_DRV_PIXEL_BITS / 8);
    ...
    LCDC_SetTxPixelLen(ST7265_800480_LCD_WIDTH * ST7265_800480_LCD_HEIGHT);
...
//    uint32_t HSA = 20, HFP = 60, HBP = 60, HACT = ST7265_800480_LCD_WIDTH;
//    uint32_t VSA = 12, VFP = 20, VBP = 30, VACT = ST7265_800480_LCD_HEIGHT;
//    uint32_t HSA = 48, HFP = 40, HBP = 40, HACT = ST7265_800480_LCD_WIDTH;
//    uint32_t VSA = 1, VFP = 13, VBP = 31, VACT = ST7265_800480_LCD_HEIGHT;
    uint32_t HSA = 4, HFP = 8, HBP = 8, HACT = ST7265_800480_LCD_WIDTH;
    uint32_t VSA = 4, VFP = 16, VBP = 16, VACT = ST7265_800480_LCD_HEIGHT;
...
//    EDPI->EDPI_RGB_COMPATIBLE = edpi_reg_0x58.d32;
...
#if ST7265_DRV_PIXEL_BITS == 24
    for (uint32_t i = 0; i < ST7265_800480_LCD_WIDTH * ST7265_800480_LCD_HEIGHT * 3; i = i + 3)
    {
        pixel[i] = 0xFF;
        pixel[i + 1] = 0;
        pixel[i + 2] = 0;
...
#endif
#if ST7265_DRV_PIXEL_BITS == 16
    for (uint32_t i = 0; i < ST7265_800480_LCD_WIDTH * ST7265_800480_LCD_HEIGHT * 2; i = i + 2)
    {
        pixel[i] = 0x00;
        pixel[i + 1] = 0x00;
    }
    ...
    rtk_lcd_hal_update_framebuffer((uint8_t *)PSRAM_FRAME_BUF1_ADDR,
                                   ST7265_800480_LCD_WIDTH * ST7265_800480_LCD_HEIGHT);
    platform_delay_ms(150);
    Pad_Config(BACKLIGHT_PIN, PAD_SW_MODE, PAD_IS_PWRON, PAD_PULL_NONE, PAD_OUT_ENABLE, PAD_OUT_HIGH);
}
...
void rtk_lcd_hal_update_framebuffer(uint8_t *buf, uint32_t len)
{
    ...
    else
    {
        LCDC_DMA_Infinite_Buf_Update(buf,
                                     (uint8_t *)buf + ST7265_800480_LCD_WIDTH * ST7265_DRV_PIXEL_BITS / 8);
    }
}
...
{
    return ST7265_800480_LCD_WIDTH;
}

uint32_t rtk_lcd_hal_get_height(void)
{
    return ST7265_800480_LCD_HEIGHT;
}

uint32_t rtk_lcd_hal_get_pixel_bits(void)
{
    return ST7265_DRV_PIXEL_BITS;
}
