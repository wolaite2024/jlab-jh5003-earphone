...
static void spic3_spi_write_command(uint8_t cmd)
{
    ...
    fmc_spic_set_user_addr_len(SPIC_ID, 0);
    fmc_spic_set_txndf(SPIC_ID, 0);

    fmc_spic_set_dr(SPIC_ID, cmd, FMC_SPIC_DATA_BYTE);
...
static void spic3_spi_write_data(uint8_t *data, uint32_t len)
{
    ...
    fmc_spic_set_cmd_len(SPIC_ID, 0);
    fmc_spic_set_user_addr_len(SPIC_ID, 0);
    fmc_spic_set_txndf(SPIC_ID, len);

    for (uint32_t i = 0; i < len; i++)
    {
        fmc_spic_set_dr(SPIC_ID, data[i], FMC_SPIC_DATA_BYTE);
    }
