...
/**
 * @brief Keyscan DLPS restore register.
 */
typedef struct
{
...
uint32_t KEYSCAN_GetDLPSBufferLen(void)
{
    return sizeof(KEYSCANStoreReg_Typedef);
}

void KEYSCAN_DLPSEnter(void *peri_reg, void *p_store_buf)
{
    KEYSCANStoreReg_Typedef *store_buf = (KEYSCANStoreReg_Typedef *)p_store_buf;
    KEYSCAN_TypeDef *KEYSCANx = (KEYSCAN_TypeDef *)peri_reg;

    store_buf->keyscan_reg[0] = KEYSCANx->CLKDIV;               /* 0x00 */
    store_buf->keyscan_reg[1] = KEYSCANx->TIMERCR;              /* 0x04 */
    store_buf->keyscan_reg[2] = KEYSCANx->CR;                   /* 0x08 */
    store_buf->keyscan_reg[3] = KEYSCANx->COLCR;                /* 0x0C */
    store_buf->keyscan_reg[4] = KEYSCANx->ROWCR;                /* 0x10 */
    store_buf->keyscan_reg[6] = KEYSCANx->INTMASK;              /* 0x18 */
}

void KEYSCAN_DLPSExit(void *peri_reg, void *p_store_buf)
{
    KEYSCANStoreReg_Typedef *store_buf = (KEYSCANStoreReg_Typedef *)p_store_buf;
    KEYSCAN_TypeDef *KEYSCANx = (KEYSCAN_TypeDef *)peri_reg;
    ...
    /* Set FSM to idle state */
    KEYSCANx->CR &= ~BIT31;
    KEYSCANx->CLKDIV = store_buf->keyscan_reg[0];
    KEYSCANx->CR = (store_buf->keyscan_reg[2] & (~(BIT31)));
    KEYSCANx->TIMERCR = store_buf->keyscan_reg[1];
    KEYSCANx->COLCR = store_buf->keyscan_reg[3];
    KEYSCANx->ROWCR = store_buf->keyscan_reg[4];
    KEYSCANx->INTMASK = store_buf->keyscan_reg[6];
    KEYSCANx->CR |= (store_buf->keyscan_reg[2] & ((BIT31)));
}
