...
/**
  ...
  * @param  GDMA_Channelx: where x can be 0 to 15 to select the DMA Channel.
...
void GDMA_Init(GDMA_ChannelTypeDef *GDMA_Channelx, GDMA_InitTypeDef *GDMA_InitStruct)
{
    ...
    if (GDMA_Channelx <= GDMA_Channel15)
    {
        base_addr = GDMA_BASE;
        channel_num = GDMA_InitStruct->GDMA_ChannelNum;
    }
    else
    {
        base_addr = GDMA1_BASE;
        channel_num = GDMA_InitStruct->GDMA_ChannelNum - GDMA1_CH_NUM0;
    }
    ...
    base_addr->DmaCfgReg = 0x01;

    /* read ChEnReg to check channel is busy or not */
    if (base_addr->ChEnReg & BIT(channel_num))
    {
        //channel is be used
        //error handle code
        //while (1);
    ...
    base_addr->CLEAR_TFR |= temp_bit;
    base_addr->CLEAR_BLOCK |= temp_bit;
    base_addr->CLEAR_DST_TRAN |= temp_bit;
    base_addr->CLEAR_SRC_TRAN |= temp_bit;
    base_addr->CLEAR_ERR |= temp_bit;
    if (channel_num < 8)
    {
        ...
        base_addr->MASK_TFR = temp_bit;
        base_addr->MASK_BLOCK = temp_bit;
        base_addr->MASK_DST_TRAN = temp_bit;
        base_addr->MASK_SRC_TRAN = temp_bit;
        base_addr->MASK_ERR = temp_bit;
    }
    else
    {

        /* mask pending interrupts of corresponding GDMA channel */
        temp_bit = BIT(channel_num + 16);
        base_addr->MASK_TFR = temp_bit;
        base_addr->MASK_BLOCK = temp_bit;
        base_addr->MASK_DST_TRAN = temp_bit;
        base_addr->MASK_SRC_TRAN = temp_bit;
    ...
    case GDMA_DIR_MemoryToMemory:
        GDMA_Channelx->CFG_LOW = ((0x03 << 10) | ((GDMA_InitStruct->GDMA_ChannelPriority & 0xF) << 4));
        break;
    case GDMA_DIR_MemoryToPeripheral:
        GDMA_Channelx->CFG_LOW = BIT11 | ((GDMA_InitStruct->GDMA_ChannelPriority & 0xF) << 4);
        break;
    case GDMA_DIR_PeripheralToMemory:
        GDMA_Channelx->CFG_LOW = BIT10 | ((GDMA_InitStruct->GDMA_ChannelPriority & 0xF) << 4);
        break;
    case GDMA_DIR_PeripheralToPeripheral:
        GDMA_Channelx->CFG_LOW = (GDMA_InitStruct->GDMA_ChannelPriority & 0xF) << 4;
        break;
    default:
        GDMA_Channelx->CFG_LOW = (GDMA_InitStruct->GDMA_ChannelPriority & 0xF) << 4;
        break;
    ...
    GDMA_Channelx->CFG_HIGH &=  ~(0x0fff << 7);
...
void GDMA_StructInit(GDMA_InitTypeDef *GDMA_InitStruct)
{
    ...
    GDMA_InitStruct->GDMA_ChannelNum      = GDMA0_CH_NUM0;
...
/**
  ...
  * @param  GDMA_Channelx: x can be 0 to 15 to select the GDMA Channel.
  * @param  NewState: new state of the DMA Channelx.
  *   This parameter can be: ENABLE or DISABLE.
  * @retval None
  */
RAM_TEXT_SECTION void GDMA_Cmd(uint8_t GDMA_ChannelNum, FunctionalState NewState)
{
    GDMA_TypeDef *base_addr;
    ...
    if (GDMA_ChannelNum <= GDMA0_CH_NUM15)
    {
    ...
    {
        GDMA_ChannelNum_ADJ = GDMA_ChannelNum - GDMA1_CH_NUM0;
        gdma_reg = (GDMA_ChannelTypeDef *)(GDMA1_REG_BASE + GDMA_ChannelNum_ADJ * 0x0058);
        base_addr = GDMA1_BASE;
    }

    uint32_t timeout = bit_need_check_times;

    if (NewState != DISABLE)
    {
        /* Enable the selected DMAy Channelx */
        base_addr->ChEnReg |= BIT(GDMA_ChannelNum_ADJ) | BIT(GDMA_ChannelNum_ADJ + 8);
    }
    else
    {
        if (base_addr->ChEnReg & BIT(GDMA_ChannelNum_ADJ))
        {
            ...
            if ((gdma_reg->CFG_LOW & GDMA_SUSPEND_CHANNEL_STATUS) != SET)
            {
        ...
        /* disable dma */
        base_addr->ChEnReg = BIT(GDMA_ChannelNum_ADJ + 8);
...
/**
  ...
  * @param  GDMA_Channelx: where x can be 0 to 15 to select the GDMA Channel.
  ...
  * @retval None
  */
RAM_TEXT_SECTION void GDMA_INTConfig(uint8_t GDMA_ChannelNum, uint32_t GDMA_IT,
                                     FunctionalState NewState)
{
    uint32_t temp_bit = 0;
    GDMA_TypeDef *base_addr = GDMA_BASE;

    if (GDMA_ChannelNum >= GDMA1_CH_NUM0)
    {
        base_addr = GDMA1_BASE;
        GDMA_ChannelNum -= GDMA1_CH_NUM0;
    }
    /* Check the parameters */
    assert_param(IS_GDMA_ChannelNum(GDMA_ChannelNum));
...
/**
  ...
  * @retval None
  */
RAM_TEXT_SECTION void GDMA_ClearINTPendingBit(uint8_t GDMA_ChannelNum, uint32_t GDMA_IT)
{
    uint32_t temp_bit = 0;
    GDMA_TypeDef *base_addr = GDMA_BASE;
    ...
    {
        base_addr->CLEAR_BLOCK = temp_bit;
        base_addr->CLEAR_BLOCK;
    }
    if (GDMA_IT & GDMA_INT_SrcTransfer)
    {
        base_addr->CLEAR_SRC_TRAN = temp_bit;
        base_addr->CLEAR_SRC_TRAN;
    }
    if (GDMA_IT & GDMA_INT_DstTransfer)
    {
        base_addr->CLEAR_DST_TRAN = temp_bit;
        base_addr->CLEAR_DST_TRAN;
    }
    if (GDMA_IT & GDMA_INT_Error)
    {
        base_addr->CLEAR_ERR = temp_bit;
